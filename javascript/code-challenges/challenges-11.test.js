...
function transformToLis(obj){
  ...
  let finalArr = newArr.map((item) => {return `<li>${item.join( `: `)}</li>`;});
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let firstArr = input.map(item => item.filter(number => (number % 5 === 0 && typeof number === 'number')));
  console.log(firstArr);
  let secondArr = firstArr.map(item => item.map(number => Math.pow(2, number)));
...
let findShortest = (data) => {
  // Solution code here...
  let newArr = data.reduce((acc, item) => parseInt(acc.height) > parseInt(item.height) ? acc = item : acc, {height: 200});
  console.log(newArr);
  return newArr.name;
};
