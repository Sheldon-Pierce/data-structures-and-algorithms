...
const validatePin = (pin) => {
  // Solution code here...
  return(/^[0-9]{4}$/).test(pin)? true : false;
};
...
const validateWord = (word) => {
  // Solution code here...
  return (/^[a-zA-Z]{5,10}$/).test(word) ? true : false;
};
...
const hasNumber = (string) => {
  // Solution code here...
  return(/[a-zA-Z]+[0-9]+/g).test(string) ? true : false;
};
...
const validateEmail = (email) => {
  // Solution code here...
  return (/^\S(?:(?:\w+\.\w+)|\w+)\@\w+\.[net|org|com]+$/).test(email) ? true : false;
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  return (/^(\(\d{3}\)|\d{3})[- ]?\d{3}[- ]?\d{4}$/).test(phoneNumber) ? true : false;
};
